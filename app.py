...
def scrape_stockanalysis_history(symbol, min_rows=150):
    ...
    NOT: Bu regex Railway konsolunda gerçek HTML üzerinde test edilmiş ve
    doğrulanmıştır (INTC için 50/50 satır doğru parse edildi).
    HTML yapısı: <td class="sym ...">Jun 24, 2026</td><td class="...">Open</td>...
    Satırlar arasında SvelteKit'in SSR yorum işaretleri (<!--[!--><!--]-->) var,
    bu yüzden [^<]* yerine .*? + re.DOTALL kullanılıyor.
    """
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
        all_rows = []
        for page in range(1, 5):
            ...
            # Doğrulanmış desen: Tarih (sym class) + 4 fiyat hücresi + hacim (virgüllü tam sayı)
            row_pattern = re.compile(
                ...
                r'<td class="[^"]*">([\d.]+)</td>.*?'
                r'<td class="[^"]*">([\d.]+)</td>.*?'
                r'<td class="[^"]*">[\d.]+</td>.*?'  # Adj. Close (atlanır)
                r'<td class="[^"]*"><span[^>]*>-?[\d.]+%</span></td>.*?'  # Change (atlanır)
                r'<td class="[^"]*">([\d,]+)</td>',  # Volume
                re.DOTALL
            )
...
/portfolio add [ticker] [price] [qty] | /portfolio""", chat_id)
    ...
    if any(t.startswith(x) for x in ['/analizet','/analiz','/analyze']):
        ...
        def do_full_analyze(tk=ticker, cid=chat_id):
            trend_result = analyze_stock(tk)
            ew_result = early_warning_scan(tk)
    ...
    while True:
        try:
            if is_nasdaq_hours():
                ...
                # Elliott Dalga ve Formasyon taraması: kredi limitini korumak için
                # daha küçük bir alt-küme üzerinde çalışır.
                pattern_batch = random.sample(batch, min(8, len(batch)))
