...
• Destek Seviyesine Yakınlık (SMA200): {"✅" if details['crit_near_support'] else "⛔"} %{details['dist_sma200']:.1f} uzaklıkta / Ref: %5 altı
...
def analyze_stock(symbol, df=None):
    try:
        ...
        data_days = len(df)
        if data_days < 20:
            print(f"[DEBUG analyze_stock] {symbol}: data_days={data_days} < 20, yetersiz veri")
            return None
        ...
        try:
            ...
            if index_perf is not None:
                ...
                rs_text = (
                    ...
                    else f"⚠️ {rs_diff:.1f}% / Ref: 0% üstü → Piyasanın Altında Kaldı"
                )
        except:
            pass
        ...
        if score < 7:
            ...
            rsi_text_i = (
                ...
                else f"⚠️ {rsi_now:.1f} / Ref: 40-65 arası → Bölge Dışı"
            )
...
def early_warning_scan(symbol, df=None):
    ...
    try:
        if df is None:
            df = td_get_ohlcv(symbol, outputsize=60)
        if df is None or len(df) < 30:
            print(f"[DEBUG early_warning_scan] {symbol}: df None veya len<30 (len={len(df) if df is not None else 'None'})")
            return None
...
"""
        ...
        if direction == "BOĞA":
            ...
            market_text = (
                f"{'✅' if crit_market else '⛔'} QQQ %{qqq_perf:.1f} / Ref: -3% üstü (Boğa) → {'Uygun' if crit_market else 'Uygun Değil'}"
                if qqq_perf is not None else "⛔ Veri yok"
            )
        else:
            crit_market = qqq_perf is not None and qqq_perf < 3
            market_text = (
                f"{'✅' if crit_market else '⛔'} QQQ %{qqq_perf:.1f} / Ref: 3% altı (Ayı) → {'Uygun' if crit_market else 'Uygun Değil'}"
                if qqq_perf is not None else "⛔ Veri yok"
            )
...
• Destek/Direnç (Ref Yakınlığı): {support_text}
...
def fetch_news_for_symbol(symbol, importance_filter=True):
    """
    stockanalysis.com/stocks/{symbol}/ sayfasındaki News bölümünden haber
    başlıklarını, özetlerini, tarihlerini ve linklerini çeker.
    ...
    try:
        ...
        results = []
        for link, title, summary, full_date, rel_date in matches[:10]:
            title = title.strip()
...
def get_scan_interval_seconds():
    ...
    - early_wake (açılışa 2 saat var): 20 dakika
    - premarket: 20 dakika
    - regular (normal seans): 20 dakika
    ...
    if session_type == "afterhours":
        ...
        return 10 * 60
    else:  # early_wake, premarket, regular
        return 20 * 60
...
def auto_scan_loop():
    time.sleep(15)
    send_telegram(f"🦅 <b>Hawk Signal Bot — NASDAQ</b>\n\n✅ Canlı piyasa taraması\n✅ Trend + Erken Uyarı sistemleri aktif\n✅ Seans içi 20 dk, kapanış sonrası 30 dk aralıkla otomatik tarama\n\nTarayıcı kütüphanesi: {'Aktif ✅' if TV_SCRAPER_AVAILABLE else 'Yedek modda ⚠️'}")
