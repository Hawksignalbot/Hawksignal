...
def check_symbol_accessible(symbol, market):
    """Bir sembolün veri kaynağında erişilebilir olup olmadığını kontrol eder"""
    try:
        result = td_get_ohlcv(symbol, market, outputsize=5)
        if result is None:
            return False
        df, _ = result
        return len(df) > 0
...
def verify_pool(market):
    ...
    NASDAQ (Finnhub): dakikada 60 istek - hızlı tarama mümkün.
    BIST/Alman (Twelve Data): dakikada 8, günde 800 istek - yavaş ve dikkatli."""
    pool = {
    ...
    wait_time = 1.2 if market == 'nasdaq' else 8
...
def finnhub_get_ohlcv(symbol, outputsize=210):
    """NASDAQ hisseleri için Finnhub'dan günlük OHLCV verisi çeker."""
    try:
        clean_symbol = symbol.upper().strip()
        end_ts = int(time.time())
        start_ts = end_ts - (outputsize + 30) * 86400  # gün hesabına tatil günleri payı
        ...
        if data.get("s") != "ok" or "c" not in data or len(data["c"]) == 0:
            return None
...
def td_get_ohlcv_raw(symbol, market, outputsize=100):
    """BIST/Alman hisseleri için Twelve Data'dan günlük OHLCV verisi çeker."""
    try:
...
def td_get_ohlcv(symbol, market, outputsize=100):
    """
    Birleşik veri çekme fonksiyonu.
    NASDAQ: Finnhub kullanılır (60 istek/dakika, bol limit).
    BIST/Alman: Twelve Data kullanılır (günlük 800 istek limiti).
    """
    if market == 'nasdaq':
        return finnhub_get_ohlcv(symbol, outputsize)
    else:
        return td_get_ohlcv_raw(symbol, market, outputsize)
...
/dogrula — Hisse listesini yeniden doğrula
────────────────────────
📈 <b>TARAMA</b>
...
/portfoy | /portfoy [hisse]

ℹ️ BIST/Alman için Şahin Sinyal botunu kullan.""", chat_id)
        return
...
ℹ️ For BIST/German signals, use Şahin Sinyal bot.""", chat_id)
        return

    if any(t.startswith(x) for x in ['/liste','/list']):
        pool_count = len(get_verified_pool('nasdaq'))
        send_telegram(f"""📋 <b>HİSSE HAVUZU — NASDAQ</b>

🇺🇸 Doğrulanmış havuz: {pool_count} hisse
...
Her tarama farklı hisselerle yapılır.""", chat_id)
    ...
    # BIST ve Alman artık Şahin Sinyal botunda - burada bilgi ver
    bist_alman_triggers = ['/bist','/turkiye','/istanbul','/turkey','/alman','/almanya','/dax','/german','/germany']
    if any(t.startswith(x) for x in bist_alman_triggers):
        send_telegram("📢 BIST ve Alman borsası sinyalleri artık <b>Şahin Sinyal</b> botuna taşındı.\n\nHawk Signal Bot sadece NASDAQ üzerine odaklanıyor.", chat_id)
        return
    ...
    if any(t.startswith(x) for x in ['/uyumsuzluk','/divergence']):
        ...
        market = 'nasdaq'  # Sadece NASDAQ destekleniyor
...
def auto_scan_loop():
    time.sleep(15)
    send_telegram("🦅 <b>Hawk Signal Bot v3.0 — NASDAQ</b>\n\n✅ 5 Kademe Filtre Aktif\n✅ Dinamik hacim tabanlı hisse seçimi\n✅ 25 dakikada bir otomatik tarama\n\nℹ️ BIST/Alman sinyalleri için Şahin Sinyal botunu kullan.")

    # Doğrulama sadece hiç yapılmamışsa otomatik başlasın (limit tasarrufu için)
    if verified_pools['nasdaq'] is None:
        send_telegram("🔍 İlk kurulum: NASDAQ hisse listesi doğrulanıyor...")
        threading.Thread(target=run_verification, args=('nasdaq', CHAT_ID)).start()
        while verification_in_progress['nasdaq']:
            time.sleep(2)

    while True:
        try:
            # Haftalık otomatik yeniden doğrulama
            last = last_verification_time['nasdaq']
            if last is None or (datetime.now() - last).days >= 7:
                if not verification_in_progress['nasdaq']:
                    threading.Thread(target=run_verification, args=('nasdaq', CHAT_ID)).start()
