...
def auto_scan_loop():
    ...
    # Manuel /dogrula komutu her zaman çalışır
    any_verified = any(verified_pools[m] is not None for m in ['nasdaq','bist','alman'])
    if not any_verified:
        send_telegram("🔍 İlk kurulum: hisse listeleri doğrulanıyor (bu işlem haftada bir otomatik tekrarlanır)...")
        for m in ['nasdaq', 'bist', 'alman']:
            threading.Thread(target=run_verification, args=(m, CHAT_ID)).start()
            time.sleep(3)
            while verification_in_progress[m]:
                time.sleep(2)
    ...
    while True:
        try:
            ...
            for m in ['nasdaq','bist','alman']:
                last = last_verification_time[m]
                if last is None or (datetime.now() - last).days >= 7:
                    if not verification_in_progress[m]:
                        threading.Thread(target=run_verification, args=(m, CHAT_ID)).start()
                        time.sleep(3)
