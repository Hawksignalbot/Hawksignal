...
CHAT_ID_TREND = os.environ.get("CHAT_ID_TREND", CHAT_ID)        # Kanal 1: Trend Sinyalleri
CHAT_ID_ERKENUYARI = os.environ.get("CHAT_ID_ERKENUYARI", CHAT_ID)  # Kanal 2: Erken Uyarı
CHAT_ID_FORMASYON = os.environ.get("CHAT_ID_FORMASYON", CHAT_ID)    # Kanal 3: Formasyon
CHAT_ID_ELLIOTT = os.environ.get("CHAT_ID_ELLIOTT", CHAT_ID)        # Kanal 4: Elliott Dalga + Otomatik Tarama
CHAT_ID_HABER = os.environ.get("CHAT_ID_HABER", CHAT_ID)            # Kanal 5: Haberler
CHAT_ID_SISTEM = os.environ.get("CHAT_ID_SISTEM", CHAT_ID)          # Kanal 6: Takip/Alarm/Sistem uyarıları
                ...
                if losers:
                    ...
                    for ticker in losers:
                        ...
                        if result and result.get("signal"):
                            send_telegram(result["signal"], CHAT_ID_ELLIOTT)
...
def translate_and_interpret_news(title, summary, symbol):
    """
    Claude API (Haiku modeli) kullanarak haber başlığını ve özetini
    Türkçe'ye çevirir ve kısa bir sentiment yorumu ekler.

    Maliyet tahmini: ~300 input + 200 output token/haber
    Aylık ~110 haber için: ~$3-5 (Haiku)

    Eğer API çağrısı başarısız olursa (rate limit, bağlantı hatası vb.)
    orijinal İngilizce metni döndürür — sessizce başarısız olur.
    """
    try:
        if not ANTHROPIC_API_KEY:
            print("[DEBUG translate_and_interpret_news] ANTHROPIC_API_KEY tanımlı değil, İngilizce fallback")
            return f"{title}\n{summary}"
        ...
        payload = {
            ...
            "messages": [{
                "role": "user",
                "content": (
                    f"Aşağıdaki haber başlığını ve özetini Türkçe'ye çevir, "
                    f"ardından bu haberin {symbol} hissesi için pozitif mi, negatif mi "
                    f"yoksa nötr mü olduğunu tek bir emoji ile belirt "
                    f"(🟢 pozitif / 🔴 negatif / ⚪ nötr). "
                    f"Format: [Çevrilmiş Başlık]\n[Çevrilmiş Özet]\n[Emoji] [Kısa yorum - max 1 cümle]\n\n"
                    f"Başlık: {title}\nÖzet: {summary}"
                )
            }]
        }
        ...
        if r.status_code == 200:
            ...
            print(f"[DEBUG translate_and_interpret_news] API hatası: {r.status_code} {r.text[:200]}")
    except Exception as e:
        print(f"[DEBUG translate_and_interpret_news] Exception: {e}")
    # Fallback: orijinal İngilizce
    return f"{title}\n{summary}"
...
def send_news_for_signal(symbol, source_label=""):
    ...
    try:
        ...
        for item in news_items[:3]:
            ...
            if item.get("link"):
                msg += f"\n🔗 {item['link']}"
            send_telegram(msg, CHAT_ID_HABER)
            mark_news_sent(nid)
    except Exception as e:
    ...
    while True:
        try:
            if is_bot_active():
                ...
                if last_pool_source["source"] == "memory":
                    send_telegram("⚠️ Otomatik tarama: canlı veri alınamadı, hafızadaki son başarılı taramaya geçildi.", CHAT_ID_SISTEM)
                elif last_pool_source["source"] == "none":
                    send_telegram("❌ Otomatik tarama: hiçbir veri kaynağına erişilemedi, bu döngü atlandı.", CHAT_ID_SISTEM)
                    time.sleep(1500)
                    continue
                ...
                for ticker in batch:
                    result = analyze_stock(ticker)
                    if result and result.get("signal"):
                        send_telegram(result["signal"], CHAT_ID_TREND)
                ...
                for ticker in ew_batch:
                    result = early_warning_scan(ticker)
                    if result and result.get("signal"):
                        send_telegram(result["signal"], CHAT_ID_ERKENUYARI)
                ...
                for ticker in pattern_batch:
                    result = formation_analysis(ticker)
                    if result and result.get("signal"):
                        send_telegram(result["signal"], CHAT_ID_FORMASYON)
