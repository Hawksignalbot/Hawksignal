...
def check_reversal_criteria(df):
    """
...
def elliott_auto_scan_candidate(symbol):
    ...
    try:
        ...
        confirmed, details = check_reversal_criteria(df)
        if confirmed < 3:
            return None
...
def fetch_news_for_symbol(symbol):
    ...
    NOT: Önceki sürüm tradingview-scraper'a (TV_SCRAPER_AVAILABLE) bağlıydı,
    bu kütüphane Railway'de hiç aktif olmadığı için fonksiyon her zaman boş
    liste döndürüyordu. Bu sürüm, OHLCV/sektör için kullandığımız aynı
    güvenli yöntemle (önce yorumları temizle, sonra basit regex ile eşleştir)
    gerçek veri çeker.
    """
    try:
        ...
        results = []
        for link, title, summary, full_date, rel_date in matches[:5]:
            results.append({
                "title": title.strip(),
...
📰 <b>HABER</b>
/haber [HİSSE] — Sembole özel haber akışı
(eş değer: /news)
...
──────────────────────────
ℹ️ Detaylı İngilizce liste için /help yazabilirsin.""", chat_id)
        return
...
📰 <b>NEWS</b>
/news [TICKER] — Symbol-specific news feed
(alias: /haber)
...
──────────────────────────
ℹ️ For the Turkish list, type /yardim.""", chat_id)
        return
